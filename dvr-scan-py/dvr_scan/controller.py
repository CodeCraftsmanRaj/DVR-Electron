...
from dvr_scan.extractor import run_extractor
...
def _preprocess_args(args):
    """Perform some preprocessing and validation of command line arguments for the 'scan' command."""
    # -i/--input
    input_files = []
...
def parse_settings() -> ty.Optional[ScanSettings]:
    ...
    try:
        ...
        if hasattr(args, 'ignore_user_config') and args.ignore_user_config:
            config_load_error = None
            config = ConfigRegistry()
...
def run_extractor_from_args(settings: ScanSettings):
    """Wrapper to call the extractor logic with parsed settings."""
    args = settings.get_arg(None) # get the whole args object
    run_extractor(args)
...
def run_dvr_scan(
    ...
    if settings.get_arg("json_output"):
        ...
        if result.event_list:
            ...
            # This logic was slightly flawed, let's fix it to create a proper timecode list.
            timecode_list = []
